...
from dolfinx_iga.splines.basis_1D import evaluate_Bspline_basis, evaluate_Lagrange_basis
from dolfinx_iga.splines.bspline_1D import (
    Bspline1D,
)
from dolfinx_iga.splines.change_basis_1D import create_Lagrange_extraction_operators
...
Cs = create_Lagrange_extraction_operators(space)
...
for i in range(space.get_num_intervals()):
    ...
    C = Cs[i]
    bases = evaluate_Lagrange_basis(degree, tt) @ C.T
    for k in range(degree + 1):
        ax1.plot(tt * (k1 - k0) + k0, bases[:, k])

    new_tt = tt * (k1 - k0) + k0
    bases_old, _ = evaluate_Bspline_basis(space, new_tt)
    for k in range(degree + 1):
